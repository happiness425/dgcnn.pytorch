...
def download_modelnet40():
    ...
    # 如果数据集已经上传，直接使用它的路径
    if os.path.exists('/content/gdrive/MyDrive/DGCNN/data/modelnet40-ply-hdf5-2048'):
...
def download_shapenetpart():
    ...
    DATA_DIR = os.path.join(BASE_DIR, 'data')
      # 如果数据集已经上传，直接使用它的路径
    if os.path.exists('/content/gdrive/MyDrive/DGCNN/data/shapenet-part-seg-hdf5-data'):
        DATA_DIR = '/content/gdrive/MyDrive/DGCNN/data/shapenet-part-seg-hdf5-data'
        print(f"Using uploaded dataset at {DATA_DIR}")
        return DATA_DIR
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
...
def download_S3DIS():
    ...
    DATA_DIR = os.path.join(BASE_DIR, 'data')
     # 如果数据集已经上传，直接使用它的路径
    if os.path.exists('/content/gdrive/MyDrive/DGCNN/data/indoor3d_sem_seg_hdf5_data'):
        DATA_DIR = '/content/gdrive/MyDrive/DGCNN/data/indoor3d_sem_seg_hdf5_data'
        print(f"Using uploaded dataset at {DATA_DIR}")
        return DATA_DIR
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
